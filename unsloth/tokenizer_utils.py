...
IGNORED_TOKENIZER_NAMES = frozenset([x.lower() for x in IGNORED_TOKENIZER_NAMES])
...
def fix_chat_template(tokenizer):
    ...
    try:
        ...
        no = tokenizer.apply_chat_template(
            messages, add_generation_prompt=False, tokenize=False, tools=None
        )
        yes = tokenizer.apply_chat_template(
...
def add_new_tokens(
    ...
    if method == "interpolation":
        ...
        for j, token in enumerate(new_tokens):
            ...
            mean_lm_head_token = (
                mean_lm_head * (1 - interpolation) + mean_lm_head_token * interpolation
            )
...
def check_nvidia():
    ...
    try:
        ...
        if not torch.cuda.is_available():
            raise RuntimeError(
                "Unsloth: We do not support AMD / Intel machines yet - it is a work in progress!"
            )
    return output
...
def patch_sft_trainer_tokenizer():
    ...
    for function_name, replacer in (
        ...
        function = getsource(
            eval(f"trl.trainer.sft_trainer.SFTTrainer.{function_name}")
        )
